...
class Solution {
public:
    void combinationSum(vector<int> nums, int i, int target, vector<int> combi, vector<vector<int>>& res){
        ...
        combinationSum(nums,i,target-nums[i],combi,res);
        combi.pop_back();
        combinationSum(nums,i+1,target,combi,res);
    }
    vector<vector<int>> combinationSum(vector<int>& nums, int target) {
        ...
        combinationSum(nums,0,target,combi,res);
        return res;
    }
};
```